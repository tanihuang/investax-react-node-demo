...
const UserLogPage = () => {
  ...
  useEffect(() => {
    const loadLogs = async () => {
      setLoading(true);
      try {
        const res = await fetch('http://ec2-13-115-241-205.ap-northeast-1.compute.amazonaws.com:3000/api/auth/logs');
        if (!res.ok) {
          throw new Error(`HTTP error ${res.status}`);
        }

        const logsFromServer = await res.json();
        setLogs(logsFromServer);
        setFilteredLogs(logsFromServer);
        setError(null);
      } catch (err) {
  ...
  const handleDelete = async (logId) => {
    ...
    try {
      const response = await fetch(`http://ec2-13-115-241-205.ap-northeast-1.compute.amazonaws.com:3000/api/auth/${logId}`, {
        method: 'DELETE',
      });
